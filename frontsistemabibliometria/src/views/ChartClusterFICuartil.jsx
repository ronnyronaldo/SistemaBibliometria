...
class Barchar extends Component {
    ...
    componentDidMount() {
        ...
        const {data, w, h, nc, fi, totales} = this.props;
        ...
        // Add X axis
        var x = d3.scaleLinear().domain([-1, fi]).range([0, width-150]);
        svg
        .append("g")
        ...
        // Add Y axis
        var y = d3.scaleLinear().domain([-1, nc]).range([ height, 0]);
        svg
        .append("g")
        .call(d3.axisLeft(y));
        ...
        const xScale = scaleLinear().domain([-1, fi]).range([0, width-150])
        const yScale = scaleLinear().domain([-1, nc]).range([height, 0])
        ...
        .enter()
        .append("circle")
        .attr("cx", d => xScale(d.id_factor_impacto))
        .attr("cy", d => yScale(d.id_cuartil))
        .attr("r", d => 5)
